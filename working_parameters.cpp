...
CString VariableConfigBlock::GetCurrentMode()
{
	...
	GetPrivateProfileString(TEXT("Mode")/*字段名*/, TEXT("CurrentMode")/*键名*/, TEXT(""), szCurConfig, _countof(szCurConfig), configPath_);
...
bool VariableConfigBlock::SetCurrentMode(LPCTSTR _currentMode)
{
	//若路径为空
	if (_tcscmp(configPath_, TEXT("")) == 0)
	{
	...
	return WritePrivateProfileString(TEXT("Mode")/*字段名*/, TEXT("CurrentMode")/*键名*/, _currentMode, configPath_);
}
