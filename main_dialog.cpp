...
BOOL MainDialog::OnInitDialog()
{
	...
	ASSERT((IDM_ABOUTBOX & 0xFFF0) == IDM_ABOUTBOX); // 低4位是系统内部使用的
	ASSERT(IDM_ABOUTBOX < 0xF000);

	CMenu* pSysMenu = GetSystemMenu(FALSE);//获得窗口菜单的拷贝
	if (pSysMenu != NULL)
	{
		BOOL bNameValid;
		...
		if (!strAboutMenu.IsEmpty())
		{
			...
			//IDM_ABOUTBOX表示要执行的动作，因为在点击这个菜单项时，
			//将会发送消息，消息中nID就是这个ID，通过比对ID产生相应的动作
	...
	//  执行此操作
	SetIcon(m_hIcon, TRUE);			// 设置大图标32*32
	SetIcon(m_hIcon, FALSE);		// 设置小图标16*16

  // 设置标题的字体、背景色及字体颜色
  labelTitle_.SetFont(GuiResources::GetInstance()->GetFont(GuiResources::FONT_NORMAL), FALSE);
  labelTitle_.SetBkColor(RGB(54, 133, 214));		// 蓝色
  labelTitle_.SetTextColor(RGB(255, 255, 255));		// 白色
